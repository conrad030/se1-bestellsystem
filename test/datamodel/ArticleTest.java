...
public class ArticleTest {
	...
	public void test001_RegularConstructor() {
		...
		assertSame( a.getDescription(), aToaster_description );
	...
	public void test003_NullArgumentConstructor() {
		...
		assertEquals( a.getId(), null );	// assert that correct id is returned
		assertSame( a.getId(), null );
		assertEquals( a.getDescription(), "" );
		assertSame( a.getDescription(), "" );
		assertEquals( a.getUnitPrice(), 0);
		assertEquals( a.getUnitsInStore(), 0);
	...
	public void test010_SetDescription() {
		...
		// test regular case
		assertEquals( aToaster.getDescription(), description );	// assert that correct id is returned
		assertSame( aToaster.getDescription(), description );
		
		aToaster.setDescription( "" );	
		// test regular case
		assertEquals( aToaster.getDescription(), "" );	// assert that correct id is returned
		assertSame( aToaster.getDescription(), "" );
	...
	public void test011_SetUnitPrice() {
		...
		assertEquals( aToaster.getUnitPrice(), price );	// assert that correct id is returned
		assertSame( aToaster.getUnitPrice(), price );
		
		aToaster.setUnitPrice( 0 );					// regular case
		
		assertEquals( aToaster.getUnitPrice(), 0 );	// assert that correct id is returned
		
		aToaster.setUnitPrice( Long.MAX_VALUE );					// regular case
		
		assertEquals( aToaster.getUnitPrice(), 0 );	// assert that correct id is returned
		
		aToaster.setUnitPrice( Long.MIN_VALUE );					// regular case
		
		assertEquals( aToaster.getUnitPrice(), -1 );	// assert that correct id is returned
	}
	...
	public void test012_SetUnitsInStore() {
		...
		assertEquals( aToaster.getUnitsInStore(), units );	// assert that correct id is returned

		aToaster.setUnitsInStore( 0 );				// regular case
		
		assertEquals( aToaster.getUnitsInStore(), 0 );	// assert that correct id is returned
		
		aToaster.setUnitsInStore( 0 );				// regular case
		
		assertEquals( aToaster.getUnitsInStore(), 0 );	// assert that correct id is returned
		
		aToaster.setUnitsInStore( Integer.MAX_VALUE );				// regular case
		
		assertEquals( aToaster.getUnitsInStore(), 0 );	// assert that correct id is returned
		
		aToaster.setUnitsInStore( Integer.MIN_VALUE );				// regular case
		
		assertEquals( aToaster.getUnitsInStore(), -1 );	// assert that correct id is returned
	}
